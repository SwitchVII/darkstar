...
mysqladmin -h localhost -u root -pmySQL@dm1n DROP dspdb

ECHO Creating Database dspdb
mysqladmin -h localhost -u root -pmySQL@dm1n CREATE dspdb

ECHO Loading dspdb tables into the database
cd C:\Program Files (x86)\DarkStar\sql
FOR %%X IN (*.sql) DO ECHO Importing %%X & "c:\program files\mysql\mysql server 5.6\bin\mysql" dspdb -h localhost -u root -pmySQL@dm1n < %%X
